...
if __name__ == '__main__':
    ...
    print("normals", n)
    # Ground truth
    im = axes[0].imshow((n+1.0)/2.0, cmap='viridis')
    cbar = plt.colorbar(im)
    axes[0].set_title('normals')
    axes[0].axis('off')
    
    im2 = axes[1].imshow(n_mask, cmap='viridis')
    cbar = plt.colorbar(im2)
    axes[1].set_title('normals mask')
    axes[1].axis('off')
    ...
    im2 = axes[3].imshow(d_mask)
    axes[3].set_title('depth mask')
    axes[3].axis('off')
