...
def load_obj(
    ...
    normals = None
    if load_normals and len(attrib.normals) > 0:
        normals = torch.FloatTensor(attrib.normals).reshape(-1, 3)
        # normal_faces = torch.LongTensor(normal_faces).reshape(-1, 3)
    ...
    if load_materials:
        ...
        return vertices, faces, texv, texf, mats, normals
